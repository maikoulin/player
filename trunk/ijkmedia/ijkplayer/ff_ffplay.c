...
//added by wym
static int get_first_error_code(int error)
{
    switch (error) {
        case IJK_CMS_AUTH_FAIL:
        case IJK_CMS_REQ_TIMEOUT:
        case IJK_CMS_INVAL_ARG:
        case IJK_CMS_KEY_ERROR:
        case IJK_CMS_UNKNOWN_ERROR:
            return IJK_MEDIA_ERROR_CMS_VERIFICATION;
        case IJK_DEMUXER_NOT_FOUND: 
            return IJK_MEDIA_ERROR_UNSUPPORTED;
        default:
            ;
    }
    return IJK_MEDIA_ERROR_TIMED_OUT;
}
//end
...
static int packet_queue_get_or_buffering(FFPlayer *ffp, PacketQueue *q, AVPacket *pkt, int *serial, int *finished)
{
    ...
    while (1) {
        ...
        if (new_packet < 0)
            ...
            //hzl start: if hls eof, need to notify error event
            if(ffp->is->ic->duration == AV_NOPTS_VALUE && ffp->error){
                print_error("wym_packet_queue_get_or_buffering", ffp->error);
                ffp_notify_msg3(ffp, FFP_MSG_ERROR, get_first_error_code(ffp->error), ffp->error);
            }
            //hzl end
...
static int decoder_decode_frame(FFPlayer *ffp, Decoder *d, AVFrame *frame, AVSubtitle *sub) {
    ...
    do {
        ...
        //added by wym
        if (d->avctx->non_increasing_dts > 0) {
            tmp_pts = d->pkt_temp.pts;
            d->pkt_temp.pts = d->pkt_temp.dts;
            d->pkt_temp.dts = tmp_pts;
        }
        //end
        ...
        if (ret < 0 /*added by wym*/&& ffp->audio_finish != 1) {
            d->packet_pending = 0;
        } else {
            d->pkt_temp.dts =
            ...
            if (d->pkt_temp.data) {
                ...
                if (!got_frame ||/*added by wym*/
                        (ret == 0 && d->avctx->codec_type == AVMEDIA_TYPE_VIDEO && ffp->audio_finish == 1)) {
                    if (ret == 0 && d->avctx->codec_type == AVMEDIA_TYPE_AUDIO) {
                        av_log(NULL, AV_LOG_DEBUG, "[%s:%d] Audio finish \n", __func__, __LINE__);
                        ffp->audio_finish = 1;
                    }
                    if (d->avctx->codec_type == AVMEDIA_TYPE_VIDEO)
                        av_log(NULL, AV_LOG_DEBUG, "[%s:%d] Video finish \n", __func__, __LINE__);
                    //ended wym
                    d->packet_pending = 0;
...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    //hzl for fix blurred screen when begin play video
    //total max delay is 150ms,maybe less 150ms if mediacodec_video_size_change increased
    if(!ffp->first_video_frame_rendered && ffp->video_codec_info &&
        strstr(ffp->video_codec_info, "OMX.amlogic")){
        int count = 0;
        do{
            SDL_Delay(10); // 10ms
            count++;
        }while((ffp->mediacodec_video_size_change < 3) && (count < 15));
    }
    //hzl end
...
static void video_display2(FFPlayer *ffp)
{
    ...
    //added by wym
    ffp->stat.vid_ref_time = av_gettime_relative() - start_time;
    //end
}
...
static void stream_seek(VideoState *is, int64_t pos, int64_t rel, int seek_by_bytes)
{
    ...
    //end
        ...
        av_log(NULL, AV_LOG_DEBUG, "seek_pos: %lld seek_rel:%lld seek_flags:%p\n", is->seek_pos, is->seek_rel, is->seek_flags);
        if (seek_by_bytes)
            is->seek_flags |= AVSEEK_FLAG_BYTE;
        if (is->seek_req == 1)
            is->seek_continue = 1;
...
display:
    ...
    is->force_refresh = 0;
    ffp->stat.frame_drops_early = is->frame_drops_early;
    ffp->stat.frame_drops_late = is->frame_drops_late;
    if (ffp->show_status) {
...
#endif
        //added by wym
        ffp->stat.soft_dec_time = av_gettime_relative() - start_time;
        //end
...
static int audio_decode_frame(FFPlayer *ffp)
{
    ...
    //hzl for fix hls seek issue
    //if hls stream, save the pkt pts if written to audiotrack, it will used to find the correct segment
    if((is->ic->iformat->name != NULL) && !strcmp(is->ic->iformat->name, "hls,applehttp"))
        is->ic->ijk_cur_pkt_pts = af->frame->pkt_pts;
    //hzl end
#ifdef FFP_SHOW_AUDIO_DELAY
    {
...
static int read_thread(void *arg)
{
    ...
    if (!wait_mutex) {
        ...
        ret = AVERROR(ENOMEM);
        //added by wym
        last_error = IJK_MEDIA_ERROR_UNKNOWN;
        last_error_2 = AVERROR(ENOMEM);
        //end
        goto fail;
    }
    ...
    if (err < 0) {
        //added by wuyumin
        last_error = get_first_error_code(err);
        if (err == IJK_DEMUXER_NOT_FOUND) {
            av_log(NULL, AV_LOG_FATAL, "[%s:%d] Unsupported format, can't find demuxer \n", __func__, __LINE__);
            //last_error = IJK_MEDIA_ERROR_UNSUPPORTED;
        } else if (err == IJK_MEDIA_ERROR_TIMED_OUT) {
            print_error(is->filename, err);
            av_log(NULL, AV_LOG_FATAL, "[%s:%d] avformat_open_input fail \n", __func__, __LINE__);
            //last_error = IJK_MEDIA_ERROR_TIMED_OUT;
        }
        last_error_2 = err;
        //end
        ret = -1;
    ...
    if (err < 0) {
        ...
        ret = -1;
        //added by wym
        last_error = IJK_MEDIA_ERROR_MALFORMED;
        last_error_2 = err;
        //end
        goto fail;
    }
    ...
    //hzl start use first audio stream index
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        //hzl start use first audio stream index
        if((audio_stream_index < 0) && (AVMEDIA_TYPE_AUDIO == type))
    ...
    if (video_stream_count > 1 && st_index[AVMEDIA_TYPE_VIDEO] < 0) {
        //hzl for maybe h264 stream is not the most clearly stream
    ...
    if (!ffp->audio_disable)
        st_index[AVMEDIA_TYPE_AUDIO] =
            av_find_best_stream(ic, AVMEDIA_TYPE_AUDIO,
                                ...
                                NULL, 0);
    //hzl start use first audio stream index
    if(st_index[AVMEDIA_TYPE_AUDIO] > audio_stream_index){
        av_log(NULL, AV_LOG_ERROR, "%s-%d: best select audio stream index = %d, we change to first audio stream %d\n", 
            __func__, __LINE__, st_index[AVMEDIA_TYPE_AUDIO], audio_stream_index);
        st_index[AVMEDIA_TYPE_AUDIO] = audio_stream_index;
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ...
        if (fps)
            ffp->stat.efps = av_q2d(st->avg_frame_rate);
        if (tbr)
            ffp->stat.tbr = av_q2d(st->r_frame_rate);
    }
    //end
#ifdef FFP_MERGE
    ...
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0) {
        ...
        if (ret < 0) {
            print_error("stream_component_open:Audio", ret);
            ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_MALFORMED, ret);
        }
    }
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ...
        if (ret < 0) {
            print_error("stream_component_open:Video", ret);
            ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_MALFORMED, ret);
        }
    }
    //end
    ...
    if (is->video_stream < 0 && is->audio_stream < 0) {
        ...
        //added by wym
        last_error = IJK_MEDIA_ERROR_UNSUPPORTED;
        last_error_2 = IJK_AV_STREAM_INVALID;
        //end
        goto fail;
    }
    //added by wym
    if (is->audio_stream < 0 && is->video_stream >= 0) {
        av_log(NULL, AV_LOG_INFO, "Failed to initialize audio stream");
        ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_UNSUPPORTED, IJK_AUDIO_STREAM_INVALID);
    }
    if (is->audio_stream >= 0 && is->video_stream < 0) {
        av_log(NULL, AV_LOG_INFO, "Failed to initialize video stream");
        ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_UNSUPPORTED, IJK_VIDEO_STREAM_INVALID);
    }
    //end
    if (is->audio_stream >= 0) {
...
//      of the seek_pos/seek_rel variables
            ...
            //modified by wuyumin for seek issue
            av_log(NULL, AV_LOG_DEBUG,
                       "before seek_min: %lld seek_target: %lld seek_max: %lld\n", seek_min, seek_target, seek_max);
            av_log(NULL, AV_LOG_DEBUG, "**********************************************************************\n");
            //end
            ...
            av_log(NULL, AV_LOG_DEBUG, "######################################################################\n");
            av_log(NULL, AV_LOG_DEBUG,
                       "after seek_min: %lld seek_target: %lld seek_max: %lld\n", seek_min, seek_target, seek_max);
            //end
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR,
            ...
            if (ffp->loop != 1 && (!ffp->loop || --ffp->loop)) {
                ...
                if (completed) {
                    ...
                    if (ffp->error) {
                        ...
                        print_error("wym_read_thread_ffp->error", ffp->error);
                        //hzl: report MEDIA_ERROR_TIMED_OUT error
                        ...
                        if (has_duration(ic)) {
                            ...
                            av_log(ffp, AV_LOG_INFO, "live stream, shouldn't complete although retrive eof\n");
                            ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_TIMED_OUT, AVERROR_EOF);
                        }
                        //end wym
                    }
                }
        ...
        if (ret < 0) {
            int pb_eof = 0;
            int pb_error = 0;
            print_error("wym_read_thread_av_read_frame", ret);
            ...
            if (pb_error) {
                ...
                //revert it by wym
                //hzl start: if hls eof, need to set error flag
                //if(ic->duration != AV_NOPTS_VALUE)
                    ffp->error = 0;
                //else
                //    ffp->error = ret; //set hls error flag
                //hzl end
            }
            if (is->eof) {
                ffp_toggle_buffering(ffp, 0);
        ...
        ffp_statistic_l(ffp);
        //added by wym
        ffp_check_av_state(ffp);
        //end

        if (ffp->packet_buffering) {
            io_tick_counter = SDL_GetTickHR();
            //hzl for shorten period of buffering checking before video render
            if (abs((int)(io_tick_counter - prev_io_tick_counter)) > ((ffp->first_video_frame_rendered == 0)?100:BUFFERING_CHECK_PER_MILLISECONDS)) {
...
FFPlayer *ffp_create()
{
    ...
    //hzl for fix blurred screen when begin play video
    ffp->mediacodec_video_size_change = 0;
    //hzl end
...
int ffp_seek_to_l(FFPlayer *ffp, long msec)
{
    ...
    //hzl for fix hls seek issue
    //if hls stream, if user seek, save the user seek offset
    //here also  save the ijk buffering state to hls demuxer
    if((ffp->is->ic->iformat->name != NULL) && !strcmp(ffp->is->ic->iformat->name, "hls,applehttp")){
        //if master clock is Not a Number, we can't use master clock to calcurate hls seek offset, use hls default seek
        if(isnan(get_master_clock(is))){
            is->ic->ijk_seek_offset = AV_NOPTS_VALUE;
            is->ic->ijk_cur_pkt_pts = AV_NOPTS_VALUE;
        }else{
            is->ic->ijk_seek_offset = milliseconds_to_fftime(msec) - milliseconds_to_fftime(ffp_get_current_position_l(ffp));
            is->ic->ijk_is_buffering_on = is->buffering_on;
        }
    }
    //hzl end
...
void ffp_toggle_buffering_l(FFPlayer *ffp, int buffering_on)
{
    ...
    if (buffering_on && !is->buffering_on) {
        ...
        is->buffering_on = 1;
        //added by wym
        is->buffering_time = av_gettime_relative();
        if (is->seek_req)
            is->seek_buffering = 1;
        //end
        stream_update_pause_l(ffp);
        ...
        is->buffering_on = 0;
        //added by wym
        is->last_buffering_time = av_gettime_relative();
        if (is->avdiff_flag) {
            int diff = is->last_buffering_time - is->buffering_time;
            is->avdiff_start_time += diff;
        }
        is->buffering_flag = 0;
        is->buffering_time = 0;
...
//added by wym
void ffp_check_av_state(FFPlayer *ffp)
{
    VideoState *is            = ffp->is;
    //check network speed
    if (is->buffering_on && !is->seek_buffering) {
        int diff = (av_gettime_relative() - is->buffering_time)/1000000;
        if (diff > 30 && SDL_SpeedSampler2GetSpeed(&ffp->stat.tcp_read_sampler)/1024 < 100
                        && is->buffering_flag != 1) {
            ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_TIMED_OUT, IJK_LOW_NETWORK_SPEED);//low network speed
            is->buffering_count = 0;
            is->buffering_flag = 1;
        } else {
            diff = (is->buffering_time - is->last_buffering_time)/1000000;
            is->buffering_count = diff < 30 ? 
                    ((!is->buffering_flag)? is->buffering_count+1 : is->buffering_count) : 0;
            if (is->buffering_count >= 5) {
                ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_TIMED_OUT, IJK_LOW_NETWORK_SPEED);//low network speed
                is->buffering_count = 0;
            }
            if (!is->buffering_flag)
                is->buffering_flag = 2;
        }
        return;
    }
    //check a/v sync and distinguish the root cause
    double avdiff = -ffp->stat.avdiff;
    //check if a > v?
    if (avdiff > 1.0) {
        if (is->avdiff_flag != 1) {
            is->avdiff_start_time = av_gettime_relative();
            is->avdiff_flag = 1;
        }
        double fps = ffp->stat.efps > ffp->stat.tbr ? ffp->stat.tbr : ffp->stat.efps;
        if (ffp->stat.soft_dec_time - (int64_t)(1000000/fps) > 5000 &&
                        is->last_frame_drops_early < is->frame_drops_early) {
            is->dec_count++;
        }
        if (ffp->stat.vid_ref_time > 20*1000 &&
                        is->last_frame_drops_late < is->frame_drops_late) {
            is->disp_count++;
        }
        if (is->avdiff_flag == 1 && av_gettime_relative() - is->avdiff_start_time >= 10000000) {
            if (!is->cap_flag) {
                ALOGE("[%s:%d] avdiff: %f dec_count: %d disp_count: %d fps: %.2f",
                                __func__, __LINE__, avdiff, is->dec_count, is->disp_count, fps);
                char module[16] = {'\0'};
                char codec[128] = {'\0'};
                sscanf(ffp->video_codec_info, "%[^,]%*s%s", module, codec);
                ALOGE("module: %s, codec: %s",  module, codec);
                if (is->disp_count >= 10 && is->dec_count >= 10) {
                    ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_UNSUPPORTED, IJK_VIDEO_DEC_DISP_HIGH);//dec and disp consume long time
                } else if (is->dec_count) {
                    ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_UNSUPPORTED, IJK_VIDEO_DEC_HIGH);//dec consume long time
                } else if ((!strcmp(module, "avcodec") && fps > 30) ||
                                (!strcmp(module, "MediaCodec") && fps > 60)) {
                    ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_UNSUPPORTED, IJK_FPS_TOO_HIGH);//fps too high, cause a/v not sync
                } else if (is->disp_count > 2) {
                    ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_UNSUPPORTED, IJK_VIDEO_DISP_HIGH);//disp consume long time
                } else {
                    ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_UNSUPPORTED, IJK_UNKNOWN_ERROR_AV);//unknown error, cause a/v not sync
                }
                is->cap_flag = 1;
            }
            is->avdiff_start_time = av_gettime_relative();
    ...
    //check if a < v?
    if (ffp->stat.avdiff > 1.0) {
        if (is->avdiff_flag != 2) {
            is->avdiff_start_time = av_gettime_relative();
            is->avdiff_flag = 2;
        }
        if (is->avdiff_flag == 2 && av_gettime_relative() - is->avdiff_start_time > 10000000) {
            if (!is->cap_flag) {
                ALOGE("[%s:%d] avdiff: %f ", __func__, __LINE__, ffp->stat.avdiff);
                ffp_notify_msg3(ffp, FFP_MSG_ERROR, IJK_MEDIA_ERROR_UNSUPPORTED, IJK_UNKNOWN_ERROR_VA);//unknown error, cause a/v not sync 
                is->cap_flag = 1;
            }
            is->avdiff_flag = 0;
            is->avdiff_start_time = av_gettime_relative();
...
void ffp_check_buffering_l(FFPlayer *ffp)
{
    ...
    if (buf_time_percent >= 0/* && buf_size_percent >= 0*/) {
        buf_percent = buf_time_percent/*FFMIN(buf_time_percent, buf_size_percent)*/;
    }
    //end
    if (buf_percent) {
    ...
    if (need_start_buffering) {
        ...
        //hzl for limit the hwm_in_ms value to 500ms before video render
        if(!ffp->first_video_frame_rendered){
            if(hwm_in_ms > 500)
                hwm_in_ms = 500;
        }
...
float ffp_get_property_float(FFPlayer *ffp, int id, float default_value)
{
    switch (id) {
        ...
        //added by wym
        case FFP_PROP_FLOAT_VIDEO_FIXED_FRAMES_PER_SECOND:
            return ffp ? ffp->stat.efps : default_value;
        case FFP_PROP_FLOAT_VIDEO_R_FRAMES_PER_SECOND:
            return ffp ? ffp->stat.tbr : default_value;
        //end
        default:
...
int64_t ffp_get_property_int64(FFPlayer *ffp, int id, int64_t default_value)
{
    switch (id) {
        ...
        //added by wym
        case FFP_PROP_INT64_VIDEO_SOFT_DEC_TIME:
            return ffp ? ffp->stat.soft_dec_time : default_value;
        case FFP_PROP_INT64_VIDEO_REFRESH_TIME:
            return ffp ? ffp->stat.vid_ref_time : default_value;
        case FFP_PROP_INT64_VIDEO_FRAME_DROP_EARLY:
            return ffp ? ffp->stat.frame_drops_early : default_value;
        case FFP_PROP_INT64_VIDEO_FRAME_DROP_LATE:
            return ffp ? ffp->stat.frame_drops_late : default_value;
        case FFP_PROP_INT64_AUDIO_IS_SUPPORT_DOLBY:
            return ffp ? (avcodec_find_decoder_by_name("ac3")!= NULL &&
                          avcodec_find_decoder_by_name("eac3")!= NULL &&
                          avcodec_find_decoder_by_name("dca")!= NULL ? 1 : 0)
                          : default_value;
        //end
        default:
            return default_value;
