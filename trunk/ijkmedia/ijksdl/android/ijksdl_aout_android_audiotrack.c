...
typedef struct SDL_Aout_Opaque {
    ...
    //hzl start
    SDL_Thread *volume_tid;
    SDL_Thread _volume_tid;
    SDL_cond *volume_cond;
    SDL_mutex *volume_mutex;    
    //hzl end
...
//hzl start
static int aout_volume_thread_n(JNIEnv *env, SDL_Aout *aout){
    if((NULL == aout) || (NULL == aout->opaque) || (NULL == aout->opaque->atrack))
        return -1;

    SDL_LockMutex(aout->opaque->volume_mutex);
    SDL_CondWaitTimeout(aout->opaque->volume_cond, aout->opaque->volume_mutex, 200);
    SDL_UnlockMutex(aout->opaque->volume_mutex);

    if((NULL == aout) || (NULL == aout->opaque) || (NULL == aout->opaque->atrack))
        return -1;

    SDL_Android_AudioTrack_set_volume(env, aout->opaque->atrack, aout->opaque->left_volume, aout->opaque->right_volume);
    return 0;
}

static int aout_volume_thread(void *arg)
{
    SDL_Aout *aout = arg;
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        ALOGE("aout_volume_thread: SDL_AndroidJni_SetupEnv: failed");
        return -1;
    }

    return aout_volume_thread_n(env, aout);
}
...
static int aout_thread_n(JNIEnv *env, SDL_Aout *aout)
{
    ...
    while (!opaque->abort_request) {
        ...
        if (opaque->need_set_volume) {
            ...
            opaque->volume_tid = SDL_CreateThreadEx(&opaque->_volume_tid, aout_volume_thread, aout, "ff_aout_volume_android");
            if (!opaque->volume_tid) {
                ALOGE("aout_thread_n: failed to create volume thread, set real volume");
                SDL_Android_AudioTrack_set_volume(env, atrack, opaque->left_volume, opaque->right_volume);
            }
            //hzl end
        }
...
static void aout_close_audio(SDL_Aout *aout)
{
    ...
    //hzl start
    SDL_LockMutex(opaque->volume_mutex);
    SDL_CondSignal(opaque->volume_cond);
    SDL_UnlockMutex(opaque->volume_mutex);
    SDL_WaitThread(opaque->volume_tid, NULL);
    opaque->volume_tid = NULL;
    //hzl end
...
static void aout_free_l(SDL_Aout *aout)
{
    ...
    if (opaque) {
        ...
        //hzl start
        SDL_DestroyCond(opaque->volume_cond);
        SDL_DestroyMutex(opaque->volume_mutex);        
        //hzl end
...
SDL_Aout *SDL_AoutAndroid_CreateForAudioTrack()
{
    ...
    //hzl start
    opaque->volume_cond  = SDL_CreateCond();
    opaque->volume_mutex = SDL_CreateMutex();    
    //hzl end
    opaque->speed        = 1.0f;
