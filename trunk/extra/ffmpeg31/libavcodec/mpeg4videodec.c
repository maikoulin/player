...
int ff_mpeg4_frame_end(AVCodecContext *avctx, const uint8_t *buf, int buf_size)
{
    ...
    if (s->divx_packed) {
        ...
        if (startcode_found) {
            ...
            //hzl for fix xvid mpeg4 A-V sync issue when use ffmpeg decoder
            //because this stream copy use more space and require more CPU power to decode.
            //now discard the stream copy if stream size large than 8KB(effect non-standard B-frames)
            if((buf_size - current_pos) <= 1024*8){
