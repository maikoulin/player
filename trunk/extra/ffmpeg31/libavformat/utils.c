...
static int parse_packet(AVFormatContext *s, AVPacket *pkt, int stream_index)
{
    ...
    while (size > 0 || (pkt == &flush_pkt && got_output)) {
        ...
        if(ff_check_interrupt(&s->interrupt_callback)){
            av_log(s, AV_LOG_ERROR, "%s[%d]: user interrupt! \n", __func__, __LINE__);
            ret = AVERROR_EXIT;
            goto fail;
        }
        //hzl end
                                                ...
                                                int64_t *, int64_t))
{
    ...
    int no_change;
    int ret;
    //added by wym revert to hzl version
    int64_t rmvb_no_seek = 0;
    //end
    ...
    no_change = 0;
    //added by wym revert to hzl version
    //set the max value to quit for rmvb
    if((s->iformat->name != NULL) && !memcmp(s->iformat->name, "rm", 2)){
        rmvb_no_seek = 1;
    }
    //end
    while (pos_min < pos_limit && !rmvb_no_seek) {//modified by wym
        av_log(s, AV_LOG_TRACE,
...
//added by wym
static int codec_not_found_stream_num(AVFormatContext *ic)
{
    int codec_not_found = 0;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    for (;;) {
        ...
        for (i = 0; i < ic->nb_streams; i++) {
            ...
            st = ic->streams[i];
            //modified by wym
            if (0/*(ic->iformat->name != NULL) && !strcmp(ic->iformat->name, "mpegts")*/) {
                AVCodecContext *enc = st->internal->avctx;
                int val = 0;
                switch (enc->codec_type) {
                    ...
                    default:
                        val = 1;  //hzl add for not video/audio streams, not care for
                        break;
                }
                av_log(NULL, AV_LOG_DEBUG, "[%s:%d] codec_type: %d codec_id: %d sample_rate: %d "
                                "channels: %d width: %d extradata_size: %d val: %d \n", \
                                __FUNCTION__, __LINE__,enc->codec_type, enc->codec_id, enc->sample_rate,
                                enc->channels, enc->width,  enc->extradata_size ,val);

                //hzl for only care for audio/video streams
                if ((enc->codec_id == AV_CODEC_ID_NONE || val == 0) &&
            ...
            if (!(st->r_frame_rate.num && st->avg_frame_rate.num) &&
                ...
                int count = (ic->iformat->flags & AVFMT_NOTIMESTAMPS) ?
                    ...
                    st->info->duration_count;
                //hzl for speed up ts loading time
                if ((count < fps_analyze_framecount) &&
                    ((ic->iformat->name == NULL) || strcmp(ic->iformat->name, "mpegts")))
                    break;
                //hzl end
            }
            //hzl for speed up ts loading time
        ...
        if (read_size >= probesize) {
            ...
            //hzl for probe more data to identify all streams(especially for acc audio)
            if((ic->iformat->name != NULL) && !strcmp(ic->iformat->name, "mpegts")){
                //not worry about too long because mpegts will exit when all streams info found,see above
                if (codec_not_found_stream_num(ic)) break;
                else
                av_log(ic, AV_LOG_DEBUG,
                       "mpegts Probe buffer size not enough to identify all streams, need probe more! \n");              
            }else{
                break;
            }
            //hzl end
        ...
        if (st->codec_info_nb_frames>1) {
            ...
            if (t >= limit) {
                ...
                if((ic->iformat->name != NULL) && !strcmp(ic->iformat->name, "mpegts")){
                    ...
                    av_log(ic, AV_LOG_DEBUG,
                           "mpegts max_analyze_duration not enough to identify all streams, need probe more! \n");              
                }else{
                    if (ic->flags & AVFMT_FLAG_NOBUFFER)
                        av_packet_unref(pkt);
                    break;
