...
struct segment {
    ...
    int64_t max_pts;  //add for save the max pkt pts from packet read of current ts segment
    //hzl end
    char *url;
    char *key;
...
typedef struct HLSContext {
    ...
    //added by wym
    char *ijk_app;                       ///< holds the address of the ijkapplication
    //end
                          ...
                          struct playlist *pls, AVIOContext *in)
{
    ...
    //end demi
    while (!avio_feof(in)) {
        //hzl for fix anr issue
        if(ff_check_interrupt(c->interrupt_callback)){
            av_log(NULL, AV_LOG_ERROR, "%s[%d]: user interrupt! \n", __func__, __LINE__);
            ret = AVERROR_EXIT;
            goto fail;
        }
        //hzl end
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            if (is_segment) {
                ...
                //hzl for fix hls seek issue
                //init to AV_NOPTS_VALUE
                seg->max_pts = AV_NOPTS_VALUE;
                //hzl end
...
//by demi
static void get_json_value(char *result, int res_len, char *json, char *json_key);
static int handle_ws_aes_response(char *response, char *key, int len);
static void generate_ws_aes_url(char * res_url, char *ori_vedio_url, char *ori_key_url);
static long long bytes_to_long(unsigned char byte[8]);
static unsigned char* long_to_bytes(long long l);
static void bytes_to_hexString(const unsigned char* source, char* dest, int sourceLen);
static void hexString_to_byte(const char* source, unsigned char* dest, int sourceLen);

void bytes_to_hexString(const unsigned char* source, char* dest, int sourceLen)
{
    short i;
...
void hexString_to_byte(const char* source, unsigned char* dest, int sourceLen)
{
    short i;
    unsigned char highByte, lowByte;
...
long long bytes_to_long(unsigned char byte[8]) 
{
    int mask = 255;
    int temp = 0;
    long long res = 0LL;
...
unsigned char* long_to_bytes(long long l) 
{
    static unsigned char byte[8];
    int i = 0;
    for (i = 0; i < 8; ++i) {
        byte[i] = (unsigned char)(int)(l >> 56 - i * 8);
    }
    return byte;
}

void generate_ws_aes_url(char * res_url, char *ori_url, char *ori_key_url)
{    
    char *p;
    char *set_value = "?setvalue=";
    p = strstr(ori_url, set_value);
    if (p) {
        char difftime[17];
        unsigned char dt[8];
        long long diff_time = 0LL;
        long long time_key = 0LL;
        ...
        p += strlen(set_value);
        strncpy(difftime, p, 16);
        difftime[16] = '\0';
        p += 16;
        strncpy(secret, p, strlen(p));

        hexString_to_byte(difftime, dt, strlen(difftime));
        diff_time = bytes_to_long(dt);
        struct sysinfo info;
        sysinfo(&info);
        time_key = info.uptime + diff_time;
        time_key *= 1000;

        sprintf(md5_src, "%s&secret=%s&time=%lld", ori_key_url, secret, time_key);
        
        uint8_t md5val[16];
        av_md5_sum(md5val, md5_src, strlen(md5_src));
        int j;
        for (j = 0; j < 16; j++){
            char md5_tmp[4] = {0};
            sprintf(md5_tmp, "%02x", md5val[j]);
            strcat(sign, md5_tmp);
        }
        sprintf(res_url, "%s&sign=%s&time=%lld", ori_key_url, sign, time_key);
    }
}

void get_json_value(char *result, int res_len, char *json, char *json_key)
{
    json += strlen(json_key);
    int len = 0;
    for (;;) {
        if (iscntrl(*json) || *json == ',' || *json == '}' || len >= res_len)
            break;
        if (isalnum(*json)){
            result[len++] = *json;        
        }
...
int handle_ws_aes_response(char *response, char *key, int len)
{
    char *p;
    char returnCode[10];
    char sign[50];
    ...
    p = strstr(response,"returnCode");
    if (p) {
        get_json_value(returnCode, sizeof(returnCode), p, "returnCode");
        if (strcmp(returnCode, "1") == 0) {
            p = strstr(response,"hlsKey");
            if (p) {
                get_json_value(key, len, p, "hlsKey");
            }

            p = strstr(response,"sign");
            if (p) {
                get_json_value(sign, sizeof(sign), p, "sign");
            }
        }

        if (strcmp(returnCode, "100") == 0)
            return -30001;

        if (strcmp(returnCode, "101") == 0)
            return -30002;

        if (strcmp(returnCode, "102") == 0)
            return -30003;

        if (strcmp(returnCode, "0") == 0)
            return -30004;
    }
...
static int open_input(HLSContext *c, struct playlist *pls, struct segment *seg)
{
    ...
    av_dict_set(&opts, "seekable", "0", 0);
    //added by wym
    av_dict_set(&opts, "ijkapplication", c->ijk_app, 0);
    //end
    ...
    if (seg->key_type == KEY_NONE) {
        ...
        if (WS_AES) {
            if (strcmp(seg->key, pls->key_url)) {
                char ws_key_url[MAX_URL_SIZE] = {0};
                char ws_aes_key[33];
                char ws_aes_response[MAX_URL_SIZE];
                
                memset(ws_aes_key, '\0', sizeof(ws_aes_key));
                generate_ws_aes_url(ws_key_url, pls->url, seg->key);
                
                AVIOContext *pb;
                if (open_url(pls->parent, &pb, ws_key_url, c->avio_opts, opts) == 0) {
                    ret = avio_read(pb, ws_aes_response, sizeof(ws_aes_response));
                    ret = handle_ws_aes_response(ws_aes_response, ws_aes_key, sizeof(ws_aes_key) - 1);
                    if (ret != 1) {
                        memset(ws_aes_key, '\0', sizeof(ws_aes_key));
                        av_log(NULL, AV_LOG_DEBUG, "Get ws_aes_key failed %d.\n", ret);
                ...
                ff_hex_to_data(pls->key, ws_aes_key);
 ...
 */
static int hls_has_duration(AVFormatContext *ic)
{
    int i;
    AVStream *st;
...
static int read_data(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    //added by wym
    int retry_count = 0;
    int retry_reload_count = 0;
    //end
    //by demi
    int same_ts_retry_count = 0;
    //end demi
...
reload:
        if (!v->finished &&
            ...
            if ((ret = parse_playlist(c, v->url, v, NULL)) < 0) {
                av_log(v->parent, AV_LOG_WARNING, "Failed to reload playlist %d retry_reload_count: %d ret: %d\n",
                       v->index, retry_reload_count, ret);
                //added by wym
                if (ff_check_interrupt(c->interrupt_callback))
                    return AVERROR_EXIT;
                if (hls_has_duration(v->parent) || retry_reload_count++ > 2) {
                    av_log(v->parent, AV_LOG_ERROR, "retry_reload_count: %d return directly \n",
                                    retry_reload_count);
                    return ret;
                }
                av_log(v->parent, AV_LOG_WARNING, "retry_reload_count: %d av_usleep \n",
                                    retry_reload_count);
                av_usleep(100*1000);
                goto reload;
            }
            retry_reload_count = 0;
            //end wym
        ...
        if (ret < 0) {
            ...
            av_log(v->parent, AV_LOG_WARNING, "Failed to open segment of playlist %d\n",
                   v->index);
            //added by wym
            if (!hls_has_duration(v->parent))
                v->cur_seq_no += 1;
            else{
                //by demi
                if (++same_ts_retry_count > 1) {
                    same_ts_retry_count = 0;
                    v->cur_seq_no += 1;
                }else
                    goto reload;
                //end demi
                                      ...
                                      int64_t timestamp, int *seq_no)
{
    ...
    int64_t start_time = c->ctx->start_time == AV_NOPTS_VALUE ?
                  0 : c->ctx->start_time;

#if 1
    av_log(NULL, AV_LOG_DEBUG, "%s -> timestamp=%"PRId64" first_timestamp=%"PRId64" pos=%"PRId64"  start_time=%"PRId64" seq_no=%d\n",
                                            __func__, timestamp, c->first_timestamp, pos, c->ctx->start_time, *seq_no);
#endif

    if (timestamp < pos) {
        *seq_no = pls->start_seq_no;
        if (timestamp == start_time || (timestamp == start_time + 1)
    ...
    //hzl for fix hls seek issue
    if(c->ctx->ijk_seek_offset != AV_NOPTS_VALUE && c->ctx->ijk_cur_pkt_pts != AV_NOPTS_VALUE){
        int64_t min_pkt_pts = INT64_MAX;
        int select_seg_no = -1;

        //use ijk pkt pts to find the correct segment nomber with max_pts of each segment saved before
        for(i = pls->cur_seq_no - pls->start_seq_no; i >= 0; i--){
            if(i < pls->n_segments){
                if(pls->segments[i]->max_pts != AV_NOPTS_VALUE
                    && c->ctx->ijk_cur_pkt_pts <= pls->segments[i]->max_pts){
                    if(pls->segments[i]->max_pts < min_pkt_pts){
                        select_seg_no = i;
                        min_pkt_pts = pls->segments[i]->max_pts;
                    }
                }else{
                    break;
                }
        ...
        //if user seek in ijk buffering on state, need to use the last cur_seg_no  before buffering on to find
        if(c->ctx->ijk_is_buffering_on && select_seg_no < 0
            && c->ctx->ijk_last_cur_seg_no >= pls->start_seq_no
            && c->ctx->ijk_last_cur_seg_no < (pls->start_seq_no + pls->n_segments)){
            for(i = c->ctx->ijk_last_cur_seg_no - pls->start_seq_no; i >= 0; i--){
                if(pls->segments[i]->max_pts != AV_NOPTS_VALUE
                    && c->ctx->ijk_cur_pkt_pts <= pls->segments[i]->max_pts){
                    if(pls->segments[i]->max_pts < min_pkt_pts){
                        select_seg_no = i;
        ...
        //if select_seg_no >=0, find
        //else, not find, use default find action
        if(select_seg_no >= 0){
            int64_t total_duration = 0;

            if(c->ctx->ijk_seek_offset < 0){
                //seek backward
                //because the pkt pts from ijk is later than ijk current rending, so need to reduce 2 segments
                for(i = select_seg_no-1; i >= 0; i--){
                    total_duration += pls->segments[i]->duration;
                    if(total_duration + c->ctx->ijk_seek_offset >= 0)
                        break;
                }
            }else{
                //seek forward
                //because the pkt pts from ijk is later than ijk current rending, so will calculation from current segment
                for(i = select_seg_no+1; i < pls->n_segments; i++){
                    total_duration += pls->segments[i]->duration;
                    if(total_duration - c->ctx->ijk_seek_offset >= 0)
                        break;
                }
            ...
            if(i < 0){
                ...
                i = 0;
            }else if(i >= pls->n_segments){
                //seek to the last segment
                i = pls->n_segments - 1;
            }

            //save the real seek timestamp
            c->ctx->ijk_real_seek_timestamp = start_time + pls->segments[i]->start_time;

            av_log(NULL, AV_LOG_TRACE, "%s seek_offset: %"PRId64" cur_pkt_pts: %"PRId64" real_seek_timestamp: %"PRId64" segment number from %d + (%d to %d) \n",
                __func__, c->ctx->ijk_seek_offset, c->ctx->ijk_cur_pkt_pts, c->ctx->ijk_real_seek_timestamp, pls->start_seq_no, select_seg_no, i);

            //return the finding segment nomber
            *seq_no = pls->start_seq_no + i;
            
            return 1;
        }else{
            av_log(NULL, AV_LOG_DEBUG, "%s can't find cur_seq by ijk_cur_pkt_pts: %"PRId64" ijk_seek_offset: %"PRId64" select_seg_no: %d, use default find \n",
                __func__, c->ctx->ijk_cur_pkt_pts, c->ctx->ijk_seek_offset, select_seg_no);
            //can't find correct segment with ijk pkt pts, reset to AV_NOPTS_VALUE, and use default find below, not return here
            c->ctx->ijk_seek_offset = AV_NOPTS_VALUE;
            c->ctx->ijk_cur_pkt_pts = AV_NOPTS_VALUE;
            c->ctx->ijk_real_seek_timestamp = AV_NOPTS_VALUE;
        }
    }
    //hzl end
...
static int select_cur_seq_no(HLSContext *c, struct playlist *pls)
{
    ...
    if (pls->finished && c->cur_timestamp != AV_NOPTS_VALUE) {
        //hzl for fix hls seek issue
        //reset value to AV_NOPTS_VALUE because of not user seek action
        c->ctx->ijk_seek_offset = AV_NOPTS_VALUE;
        c->ctx->ijk_cur_pkt_pts = AV_NOPTS_VALUE;
        //hzl end
...
static int hls_read_header(AVFormatContext *s)
{
    ...
    if (u) {
        ...
        //added by wym
        // get the previous ijkapplication & set back to null if string size is zero
        update_options(&c->ijk_app, "ijkapplication", u);
        //end
    }
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (ret < 0) {
            //added by wym
            if (ret == -10001 && pls->segments[0]->key_type == KEY_AES_128) {
                ret = -30005; //consisder this as key error
            }
...
static int hls_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (minplaylist >= 0) {
        ...
        *pkt = pls->pkt;
        ...
        //hzl for fix hls seek issue
        //save pkt pts to it's segment struct
        if(pls->cur_seq_no >= pls->start_seq_no
            && pls->cur_seq_no < (pls->start_seq_no + pls->n_segments)
            && pkt->pts != AV_NOPTS_VALUE)
            pls->segments[pls->cur_seq_no - pls->start_seq_no]->max_pts = pkt->pts;
        //hzl end
        return 0;
    }
    //modified by wym
                               ...
                               int64_t timestamp, int flags)
{
    ...
    c->ctx->ijk_real_seek_timestamp = AV_NOPTS_VALUE;
    //if ijk not in buffering on state, save the current segment no
    if(!c->ctx->ijk_is_buffering_on)
        c->ctx->ijk_last_cur_seg_no = seek_pls->cur_seq_no;
    //hzl end
    ...
    //hzl for fix hls seek issue
    //update seek_timestamp with ijk real seek timstamp
    if(c->ctx->ijk_real_seek_timestamp != AV_NOPTS_VALUE){
        seek_timestamp = c->ctx->ijk_real_seek_timestamp;
        c->ctx->ijk_real_seek_timestamp = AV_NOPTS_VALUE;
    }
    //hzl end
